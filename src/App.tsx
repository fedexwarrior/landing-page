...
import { Crown, Loader2, X, Shield } from 'lucide-react';
...
function MainApp() {
  ...
  const { initiateCheckout, creditPackages, isLoadingCheckout } = useCredits();

  const [user, setUser] = useState<AuthedUser | null>(null);
  const [authChecked, setAuthChecked] = useState(false);
  const [showSignUpModal, setShowSignUpModal] = useState(false);
  const [signUpReason, setSignUpReason] = useState<string | undefined>(undefined);
  // Whatever the person was trying to do gets retried automatically after they sign in.
  const [pendingAction, setPendingAction] = useState<(() => void) | null>(null);

  // Check for an existing session on load (e.g. they signed in before and came back).
  useEffect(() => {
    fetch('/api/auth/me')
  ...
  if (!authChecked) {
    // Avoid flashing the wrong signed-in/out state while we check.
    ...
    <div className="min-h-screen bg-bg-primary relative overflow-hidden">
      ...
      {showTopUpModal && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -20 }} className="w-full max-w-md glass-strong rounded-2xl border-gold/30 p-6 relative animate-scale-in">
            <button onClick={() => setShowTopUpModal(false)} className="absolute top-4 right-4 p-1 glass-medium rounded-xl text-zinc-400 hover:text-white hover:bg-glass-strong transition">
              <X className="w-5 h-5" />
            </button>
            <div className="text-center mb-6">
              <motion.div className="w-16 h-16 mx-auto mb-4 glass-medium rounded-2xl flex items-center justify-center border-gold/30" animate={{ scale: [1, 1.05, 1] }} transition={{ duration: 2, repeat: Infinity }}>
                <Crown className="w-8 h-8 text-gold" />
              </motion.div>
              <h2 className="font-display font-medium text-2xl text-white mb-2">Top Up Credits</h2>
              <p className="text-zinc-400">Each message costs 5 credits</p>
            </div>
