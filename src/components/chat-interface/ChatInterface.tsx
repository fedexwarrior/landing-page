    ...
    <div className="flex flex-col h-screen bg-bg-primary relative overflow-hidden">
      ...
      <AnimatePresence>
        {showMenu && (
          ...
          >
   <button onClick={async () => { try { const targetUrl = window.location.origin + '/create-checkout-session'; console.log("Hitting URL:", targetUrl); const r = await fetch(targetUrl, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ packageId: 'pro' }) }); const d = await r.json(); if (d.url) { window.location.href = d.url; } else { alert('Error: ' + JSON.stringify(d)); } } catch (e: any) { alert('Fetch failed: ' + e.message); } }} className="w-full px-4 py-2 text-left flex items-center gap-2 hover:bg-zinc-800 transition-colors">
